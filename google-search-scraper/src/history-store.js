    ...
    .map((f) => {
      try {
        ...
        return {
          ...
          query: data.filters?.query ?? data.query ?? "",
          location: data.filters?.location ?? "",
          totalCollected: data.totalCollected ?? data.results?.length ?? 0,
...
export function updateResultRemarks(runId, resultId, remarks) {
  const run = getExtractionRun(runId);
  if (!run) return null;

  const idx = (run.results ?? []).findIndex((r) => r.id === resultId);
  if (idx < 0) return null;

  run.results[idx].remarks = remarks ?? "";
  run.results[idx].remarksUpdatedAt = new Date().toISOString();
  saveExtractionRun(run);
  return run.results[idx];
}
